import type {SidebarsConfig} from '@docusaurus/plugin-content-docs';

// This runs in Node.js - Don't use client-side code here (browser APIs, JSX...)

/**
 * Creating a sidebar enables you to:
 - create an ordered group of docs
 - render a sidebar for each doc of that group
 - provide next/previous navigation

 The sidebars can be generated from the filesystem, or explicitly defined here.

 Create as many sidebars as you want.
 */
const sidebars: SidebarsConfig = {
  // By default, Docusaurus generates a sidebar from the docs folder structure
  tutorialSidebar: [
    'getting-started',
    'why-promptpack',
    'regulated-environments',
    {
      type: 'category',
      label: 'Specification',
      items: [
        'spec/versions',
        'spec/overview',
        'spec/structure',
        'spec/architecture-patterns',
        'spec/examples',
        'spec/file-format',
        'spec/schema-reference',
        'spec/schema-guide',
        {
          type: 'category',
          label: 'Archive',
          collapsed: true,
          // Superseded spec versions live here rather than as seven sibling
          // categories beside the current spec pages. Each version keeps its
          // own collapsed category inside; the labels drop '(Archived)' since
          // the parent already says it.
          items: [
            {
              type: 'category',
              label: 'v1.4.1',
              collapsed: true,
              items: [
                'spec/v1.4.1/overview',
                'spec/v1.4.1/structure',
                'spec/v1.4.1/architecture-patterns',
                'spec/v1.4.1/examples',
                'spec/v1.4.1/file-format',
                'spec/v1.4.1/schema-reference',
                'spec/v1.4.1/schema-guide',
              ],
            },
            {
              type: 'category',
              label: 'v1.4.0',
              collapsed: true,
              items: [
                'spec/v1.4.0/overview',
                'spec/v1.4.0/structure',
                'spec/v1.4.0/architecture-patterns',
                'spec/v1.4.0/examples',
                'spec/v1.4.0/file-format',
                'spec/v1.4.0/schema-reference',
                'spec/v1.4.0/schema-guide',
              ],
            },
            {
              type: 'category',
              label: 'v1.3.1',
              collapsed: true,
              items: [
                'spec/v1.3.1/overview',
                'spec/v1.3.1/structure',
                'spec/v1.3.1/architecture-patterns',
                'spec/v1.3.1/examples',
                'spec/v1.3.1/file-format',
                'spec/v1.3.1/schema-reference',
                'spec/v1.3.1/schema-guide',
              ],
            },
            {
              type: 'category',
              label: 'v1.3',
              collapsed: true,
              items: [
                'spec/v1.3/overview',
                'spec/v1.3/structure',
                'spec/v1.3/architecture-patterns',
                'spec/v1.3/examples',
                'spec/v1.3/file-format',
                'spec/v1.3/schema-reference',
                'spec/v1.3/schema-guide',
              ],
            },
            {
              type: 'category',
              label: 'v1.2',
              collapsed: true,
              items: [
                'spec/v1.2/overview',
                'spec/v1.2/structure',
                'spec/v1.2/examples',
                'spec/v1.2/file-format',
                'spec/v1.2/schema-reference',
                'spec/v1.2/schema-guide',
              ],
            },
            {
              type: 'category',
              label: 'v1.1',
              collapsed: true,
              items: [
                'spec/v1.1/overview',
                'spec/v1.1/structure',
                'spec/v1.1/examples',
                'spec/v1.1/file-format',
                'spec/v1.1/schema-reference',
                'spec/v1.1/schema-guide',
              ],
            },
            {
              type: 'category',
              label: 'v1.0',
              collapsed: true,
              items: [
                'spec/v1.0/overview',
                'spec/v1.0/examples',
                'spec/v1.0/file-format',
                'spec/v1.0/schema-reference',
                'spec/v1.0/schema-guide',
              ],
            },
          ],
        },
      ],
    },
    {
      type: 'category',
      label: 'Guides',
      items: [
        'guides/add-workflow',
        'guides/add-composition',
        'guides/setup-agents',
        'guides/add-skills',
        'guides/add-evals',
      ],
    },
    {
      type: 'category',
      label: 'Processes',
      items: [
        'processes/rfc-process',
        'processes/governance',
        'processes/contributing',
      ],
    },
    {
      type: 'category',
      label: 'RFCs',
      items: [
        {
          type: 'autogenerated',
          dirName: 'rfcs',
        },
      ],
    },
    {
      type: 'category',
      label: 'Ecosystem',
      items: [
        'ecosystem/promptkit-runtime',
        'ecosystem/arena-testing',
        'ecosystem/integrations',
        'ecosystem/community-tools',
      ],
    },
    'adopters',
  ],
};

export default sidebars;
